...
Version `0.6.2` separates the fund-level Origination Pipeline control plane from measurable Sourcing Line experiments and durable Origination Candidate records. It adds a dedicated, integration-aware Origination Manager, chat-first draft line creation, machine-readable experiment starters, namespaced project relationships, line-owned schedules and source receipts, candidate-owned screening and outreach state, and an explicit migration path for legacy all-in-one pipeline instances.

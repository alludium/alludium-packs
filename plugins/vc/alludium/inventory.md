...
- `vc_sourcing_line` version `0.2.2` owns one measurable, active-Fund-specific source/screen/cadence experiment and its receipts, with `fund_id` available to the allowlisted navigation projection and Fund-keyed candidate scoring retained on each provenance relationship.
- `vc_origination_candidate` version `0.2.1` owns one company's durable pre-Deal evidence, decisions, outreach context, and multi-line provenance without collapsing Fund-relative scores into Candidate-wide fields.

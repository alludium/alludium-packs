...
**Version**: 0.5.48
**Status**: Platform-backed multi-Fund declarations

Version `0.5.48` binds workspace chat to Pipeline Manager, declares native Fund collection
settings, sources Deal Fund choices from active `vc.funds` records, and allowlists `fund_id` for
bounded navigation metadata.

This release follows `0.5.47`, which introduced the Pack-owned Deal and Pipeline Manager coordination contracts.

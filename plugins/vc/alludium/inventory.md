...
- Task templates advertise `vc_deal_room`, `vc_origination_pipeline`, `vc_sourcing_line`, `vc_origination_candidate`, `vc_investment_management`, or a combination according to their declared workflow scope; all five project types are included in this pack's `projectTypes` surface.
...
- `vc_origination_pipeline` captures upstream sourcing, candidate engagement, and candidate promotion as a separate project type and now keeps version `0.3.2`.

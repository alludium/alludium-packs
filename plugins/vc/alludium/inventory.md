...
Version `0.6.4` gives the dedicated Origination Manager dynamic access to the user's live connected applications so integrations connected after deployment can be discovered and activated in the same chat. It builds on the manager-first behavior in `0.6.3` and the measurable Sourcing Line and durable Origination Candidate model introduced in `0.6.2`.

---
id: vc.document.team_evaluation_template
title: Team Evaluation Template
documentType: template
supportedProjectTypes:
  - vc_deal_room
summary: Output template for lightweight team and founder evaluation before formal diligence.
---

# Team Evaluation Template

## Rendered Artifact Contract

When this template is used to create a task output artifact:

- Create the final artifact with `artifact_createTextArtifact`.
- Use a `.html` filename and `mimeType: "text/html"`.
- Set `content` to a complete standalone safe static HTML document beginning with `<!doctype html>`.
- Convert this Markdown scaffold into semantic HTML sections, tables, and lists with small inline CSS in a `<style>` block.
- Do not create `.md` or `text/markdown` artifacts for rendered outputs.
- Do not copy HTML into structured task output fields; those fields remain plain text or artifact UUIDs.


## Workstream Header

| Field | Content |
| --- | --- |
| Company |  |
| Evaluation run | First run / rerun |
| New evidence since prior run |  |
| Decision supported | Continue evaluation / targeted diligence / decision review / pass / watch |

## Current Team View

| Area | View | Evidence | Confidence | Gap |
| --- | --- | --- | --- | --- |
| Founder-market fit |  |  | High / Medium / Low |  |
| Relevant execution evidence |  |  | High / Medium / Low |  |
| Role coverage |  |  | High / Medium / Low |  |
| Hiring gaps |  |  | High / Medium / Low |  |
| Adviser or board quality |  |  | High / Medium / Low |  |
| Integrity or consistency signals |  |  | High / Medium / Low |  |

## Main Team Hypothesis

State the founder, execution, hiring, or governance belief that must be true for the opportunity to proceed.

## Founder Fact Table

| Person | Role | Verified Facts | Source | Confidence |
| --- | --- | --- | --- | --- |
|  |  |  |  | High / Medium / Low |

## Claims And Verification Gaps

| Claim | Current Support | Gap | Decision Impact |
| --- | --- | --- | --- |
|  | Verified / Partially supported / Founder claim / Unsupported |  |  |

## Gating Team Risk

State the team risk most likely to change the investment posture.

## Next Proof Needed

| Priority | Evidence Request / Question | Owner | Success Threshold |
| --- | --- | --- | --- |
| Must answer / Should answer / Optional |  |  |  |

## Human Judgment Prompts

List the founder, team, integrity, coachability, or relationship judgments that should remain with investors.

## Decision Impact

| Recommendation | Rationale | Conditions |
| --- | --- | --- |
| Continue / Watch / Pass / Targeted diligence / Decision review |  |  |

## Standard

Separate founder claims, verified evidence, inference, and missing proof. Do not imply completed references, background checks, or character conclusions unless authorized source results are available.

...
- copy the contract's `:root` token alias block into the artifact stylesheet and use `--color-primary`, `--color-primary-strong`, `--color-secondary`, `--color-secondary-surface`, `--color-text`, `--color-muted-text`, `--color-card`, `--color-page`, and `--color-border`
- use the shared `.document`, `.document-header`, `.summary-grid`, `.section`, `.badge`, `.badge.secondary`, `.metadata`, `.eyebrow`, `.note`, and table styles unless a narrower intake layout requires a small extension
- do not build a one-off visual system with hardcoded hex colors such as `#1a1a2e`, `#6366f1`, `#f59e0b`, or unrelated status palettes
...
Also inspect the final HTML source for the shared visual contract. If it does not contain `--color-primary`, `--color-secondary`, `.document`, `.document-header`, `.summary-grid`, and `.section`, revise it before saving. Do not save an intake artifact that falls back to ad hoc hex colors or a custom status-card-only layout.

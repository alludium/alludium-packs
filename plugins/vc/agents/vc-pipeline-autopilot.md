...
Read the created task through `task-management.getTaskDetail` before reporting success. Report only the human-readable task title, status, human owner, executing agent, and returned Open task action; never expose internal identifiers or claim success from tool intent alone. Work that needs attention must produce persisted task output, an explicit question, or a review gate; a chat-only response is not a completed durable task.

...
- Also include a short human-readable summary covering: Intake Readiness Status, Missing Information Summary, Hydrated Field Map Summary, Source Index Summary, Canonical Company Name, Company Domain, Company Logo URL. Do not output raw JSON unless the user explicitly asks for machine-readable data.
...
- Any attached pitch deck has finished processing and is inspected and cited, or is recorded as unreadable and excluded from readiness evidence; intake does not complete while a supplied deck is still polling.
- Hydrated project fields list provenance and confidence without using public-web enrichment.
- The confirmed canonical company name is emitted when it improves or normalizes the submitted project name.
- Confirmed company domain and optional logo URL are emitted when available, without making logo enrichment a readiness requirement.

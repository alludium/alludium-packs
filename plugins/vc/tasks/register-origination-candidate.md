...
Normalize one reviewed source result into one new candidate. For a run-derived registration, preserve the exact originating `run-vc-sourcing-pipeline` task ID in the reviewed input sourcing run task id; it must belong to the same Sourcing Line. Standalone/manual registration omits this optional input and remains valid, but is not counted as an outcome of a sourcing run. Never invent a run ID, substitute a child task ID, infer attribution from a Line relationship, or write system-owned attribution metadata. If supplied attribution is rejected, correct the reviewed run identity rather than silently dropping it. Capture company identity, stable dedupe key, source evidence, source record keys, discovery timestamp, and why the result matched the source line. Before proposing creation, call `project.listNavigation` with `projectTypeKey: vc_origination_candidate`, `fieldFilters: [{"key":"candidate_key","operator":"equals","values":[normalized_candidate_key]}]`, and `limit: 20`, once for `collection: active` and once for `collection: portfolio`. These two bounded server-side lookups cover readable current, passed, promoted, and archived Candidates; reject a continuation cursor and do not inspect every workspace project. If one or more exact Candidates are returned, do not emit a `projectCreation` proposal and do not complete this creation task; return the matching Candidate links and route the Sourcing Line Manager to `link-existing-origination-candidate`. If either lookup fails, returns a continuation cursor, or is ambiguous, fail closed and create nothing. For a genuinely new Candidate, emit the `projectCreation` output object with `fieldValues` at its root (text fields must be strings) and a relationships array containing an incoming `vc.sourcing_line_originated_candidate` relationship from sourcing line project id. Do not persist an exclusive owner-line or hub ID on the candidate. Multiple sourcing lines may independently relate to the same candidate through the dedicated existing-candidate link task rather than by duplicating the candidate. Do not infer a Deal Fund from any provenance line. Use `definitionJson.documentRefs` as the durable document reference contract; apply operating guidance and policy references to evidence capture, dedupe, and relationship handling.

## Available Context

- Use any supplied task context, attached files, source links, meeting notes, CRM/source records, and prior artifacts.
- Especially look for: Company Name, Sourcing Line Project ID, Source Evidence, Originating Sourcing Run Task ID.

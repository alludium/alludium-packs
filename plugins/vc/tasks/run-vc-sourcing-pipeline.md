...
When running in a Sourcing Line, call `project.getAgentContext` for the task's exact project and read its persisted fund id; do not trust optional task-seeded Fund context. Require that exact fund id to match one active record in `vc.funds`, use only that Fund mandate, and keep receipts and candidate proposals line-scoped. Never require an Origination Pipeline hub. Run only approved origination sources and preserve the reference workflow order across Companies House recent and mature windows, GitHub builder signals, X/Twitter builder signals, cheap enrichment, Affinity relationship check, first verdict, LinkedIn company enrichment only for Meet or Watch, second verdict for fresh LinkedIn company data, sync proposal, portfolio-overlap review, screen, and outreach drafts. LinkedIn people discovery is weekly by default or explicit override only. Resolve each enabled child definition with `task-definitions.list` and `task-definitions.findById`, then call `task-management.createTaskFromDefinition` with the current sourcing-run task as `parentTaskId`, the exact Sourcing Line as `projectId`, and only the reviewed inputs for that source/action boundary. Do not claim a child exists until the platform supplies the returned task ID, and do not create disabled children. For a new run-derived Candidate, carry this exact sourcing-run task ID as sourcing run task id in the reviewed inputs passed to the supported guided Register launcher. Discover it with `task-management.listInboxStarterTasks`, selecting the enabled create task starter whose `projectTypeKey` is vc origination candidate and `definitionSlug` is `register-origination-candidate`. Call `task-management.openInboxStarterTask` with its returned id as `starterId`, a new UUID `attemptId` retained unchanged for retries of the same reviewed inputs, and input containing company name, source evidence, sourcing line project id, and sourcing run task id. Stop and report the missing launcher if no matching enabled starter is available. Do not call `createTaskFromDefinition` directly for run-derived Candidate registration; do not substitute a source child task ID or create an untracked registration task. When dedupe finds an exact existing Candidate, create the predefined `link-existing-origination-candidate` task with this exact sourcing-run task as `parentTaskId` and carry the same ID as sourcing run task id; do not launch guided Candidate creation or drop the duplicate from the run receipt. Manual registrations and standalone links without an originating run omit this optional input and remain outside per-run counts. Save the platform-returned receipt, candidate batch, and source-state artifact IDs in this exact run task's output. Do not call `project.update` to write last run status, new candidates last run, last run at, or latest-run artifact summary fields on the Line. Platform reporting derives lifecycle and proposed/created/reused/rejected/failed outcomes from attributed task and finalization records, and artifact links from this run's output. A candidate in a batch or a `projectCreation` proposal is not a created Candidate. Treat the compatibility new candidates count output as proposed new candidates only; never present it as a committed creation count. Missing finalization evidence means unknown, not success. Complete only after child work and the required human review; save source outcome limitations honestly without declaring the task lifecycle yourself.
...
- Also include a short human-readable summary covering: Run Status, Proposed New Candidates Count, Promotion Ready Count, Run Completed At. Do not output raw JSON unless the user explicitly asks for machine-readable data.

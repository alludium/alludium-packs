...
Plan, then call orchestrate.delegate once with one source_collection job for Brave and one for SerpAPI. Evaluate only their bounded envelopes. Retry one failed source once at most. Delegate normalize_score with artifact references, decide accept, flag, or insufficient, then delegate draft_receipt with references, reconciled counts, and the decision. The draft_receipt worker persists the receipt. Call orchestrate.finish with decisions and artifact IDs. Never call a connector, artifact read, persistence, domain, or mutation tool from the brain.

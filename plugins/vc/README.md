...
project action. Runtime mutation support depends explicitly on Platform issue `#3589` exporting
`project.createFromChat` and
`project.applyPortfolioOperations`; the Pack allowlists only those bounded portfolio mutations and
does not bind generic `project.create` or `project.update` as a substitute.
...
fixtures first published in `0.5.49`. Their catalog and package provenance are re-pinned to `v0.6.9`
...
- Deal Pipeline, Deal Execution, Sourcing Line, and Origination Candidate project type definitions in `alludium/project-types/`
...
The VC task-definition templates advertise the four active project types: `vc_deal_room`, `vc_investment_management`, `vc_sourcing_line`, and `vc_origination_candidate`. The `v0.6.9` surface requires paired platform ingest and projection support, and Pipeline Manager's mutation contract additionally requires the bounded Deal-operation exports from Platform issue `#3589`, before it can be used as the runtime source of truth.
...
| Generated agents          | `agents/`                             | 16 agent Markdown compatibility artifacts generated from Alludium runtime YAML          |
| Generated tasks           | `tasks/`                              | 95 task prompt Markdown files generated from task-definition YAML                        |
| Project blueprints        | `project-blueprints/`                 | 4 generated project-stage/task/agent/skill blueprints                                   |
| Agent templates           | `alludium/agent-templates/`           | 16 Alludium runtime templates using the `vc_*` baseline                                  |
| Task definition templates | `alludium/task-definition-templates/` | 94 VC workflow, integration-management, and origination task templates plus catalog metadata |
| Project types             | `alludium/project-types/`             | Deal Pipeline, Deal Execution, Sourcing Line, and Origination Candidate definitions |

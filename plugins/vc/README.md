...
Version `0.6.5` aligns the Origination Pipeline manager boundary with its direct connected-application access, so bounded discovery, previews, approved on-demand runs, and enrichment can stay in the manager chat while experiment state remains in dedicated sourcing lines. It builds on the `0.6.4` dynamic connected-application access, the `0.6.3` manager-first interaction model, and the `0.6.2` split between the fund-level Origination Pipeline, first-class Sourcing Line experiments, and Origination Candidate records.

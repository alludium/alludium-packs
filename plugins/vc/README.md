...
Version `0.6.28` preserves optional sourcing-run attribution for Candidate registration and derives run outcomes from committed Platform results. Manual registration remains supported without an invented run. Requires the paired Platform #4073 contract.
...
existing platform READ_ONLY policy: trusted classified reads may execute; write, destructive,
unclassified, and malformed operations are denied before the provider. Generic approval/HITL
is not introduced. Activation history is only a hint and requires fresh authorization each turn.
Real provider and hosted validation are release gates; this declaration alone is not evidence
that those journeys have passed.
...
values remain separately labelled non-scoring context, and Deals without a confirmed Fund produce no
Fund-relative scorecard.
...
The project type version advances to `1.0.5` because `1.0.4` was published in `v0.6.18`.
...
navigation, or handoff guidance. It carries forward the corrected historical `vc_deal_room` lifecycle
recipes published in `v0.6.16`, so Platform convergence uses the inherited migration contract without
re-publishing it.
...
deployed historical versions. The release also carries forward the simplified VC Deal Pipeline
Manager's explicit read-only Affinity, Harmonic, and Exa integration surface, user-configured
connection resolution, and exclusion of unrelated connected applications.

Version `0.6.10` adds `vc_deal_pipeline` as a workspace-selectable alternative with Screening, Evaluation,
Decision, and Term Sheet statuses; four manually started, stage-independent living-document tasks;
replaceable source-neutral default criteria and templates; and a minimal Deal Manager plus Deal
Analyst model. The workspace Pipeline Manager creates the new type through the bounded chat-native
...
fields, duplicate ambiguity, and material inferred values stay in focused chat clarification. Creation
preserves source provenance and artifacts, Deal Manager receives a concise Pipeline Manager-attributed
handoff, and successful mutations return a one-sentence readback with the Platform-rendered Open
project action. Runtime mutation support depends on Platform exporting `project.createFromChat` and
`project.applyPortfolioOperations`; the Pack allowlists only those bounded portfolio mutations.
...
keeps the readiness report as a compact standalone HTML artifact, and waits once for supplied artifacts
that are still processing. Confirmed domains may produce a deterministic first-party favicon URL without
making logo availability a readiness requirement.

Version `0.6.6` made Origination hub-free and projection-first. It adds Fund-specific Sourcing Lines,
...
fixtures first published in `0.5.49`. Their catalog and package provenance are re-pinned to `v0.6.28`
...
- existing and simplified Deal Pipeline, Deal Execution, Sourcing Line, and Origination Candidate project type definitions in `alludium/project-types/`
- VC project-type document sources in `alludium/documents/`
- released ontology component packages in `alludium/ontology-components/`
...
selects exactly one Deal Pipeline type. The `v0.6.24` surface requires paired Platform ingest,
...
| Agent templates           | `alludium/agent-templates/`           | 18 Alludium runtime templates using the `vc_*` baseline                                  |
| Task definition templates | `alludium/task-definition-templates/` | 100 VC workflow, integration-management, and origination task templates plus catalog metadata |
| Project types             | `alludium/project-types/`             | Existing and simplified Deal Pipeline, Deal Execution, Sourcing Line, and Origination Candidate definitions |
| Documents                 | `alludium/documents/`                 | Pack-native methodology, SOP, checklist, template, and style-guide sources               |
| Ontology components       | `alludium/ontology-components/`       | Canonical release-pinned components and fixture package manifests for KMC composition     |
...
The ontology component surface is Packs-owned semantic content. Its catalog pins exact package and
component versions, canonical SHA-256 digests, release provenance, dependency closure, lifecycle,
compatibility, and stage-purpose bindings. Consumers must resolve exact references; `latest`,
embedded provider prompts, credentials, and unbounded provider slices are rejected. The included
finance and operations packages are public-synthetic contract fixtures, not an ontology-quality
claim or platform activation.

...
Version `0.6.25` configures Deal Manager, Pipeline Manager, and Deal Analyst to discover tools from
all connected applications while allowing only operations with the Platform's durable `read`
classification to execute. It carries forward the bounded Fund-fit scoring and dormant governed
financial-workbook declaration from `0.6.24`; the workbook declaration remains inactive until the
paired Platform ingest and execution route is released.
...
fixtures first published in `0.5.49`. Their catalog and package provenance are re-pinned to `v0.6.25`
...
selects exactly one Deal Pipeline type. The `v0.6.25` surface requires paired Platform ingest,

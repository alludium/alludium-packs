...
Version `0.6.13` advances `vc_deal_pipeline` to project-type version `1.0.2` so Platform task-routing
metadata is installed as a new immutable version instead of colliding with previously imported
`1.0.1` records. It carries forward the explicit read-only Affinity, Harmonic, and Exa integration
surface added for the simplified Deal Manager in `0.6.12`.
...
fixtures first published in `0.5.49`. Their catalog and package provenance are re-pinned to `v0.6.13`
...
The VC task-definition templates advertise five project types: `vc_deal_room`, `vc_deal_pipeline`, `vc_investment_management`, `vc_sourcing_line`, and `vc_origination_candidate`. Workspace configuration selects exactly one Deal Pipeline type. The `v0.6.13` surface requires paired Platform ingest, task-routing, and bounded chat-operation support before it can be used as the runtime source of truth.

...
`WEB_SEARCH` capability bundle. Generic effect classification and human approval for external
writes, sends, or deletes are not part of this release, so the initial pilot must keep its enabled
application inventory deliberately controlled.
...
fixtures first published in `0.5.49`. Their catalog and package provenance are re-pinned to `v0.6.24`

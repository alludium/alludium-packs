...
Version `0.6.19` carries forward the `v0.6.18` Deal ownership and historical migration corrections, and adds complete presentation metadata for the simplified Deal Pipeline document library. The paired Platform settings surface can use stable roles and investment-native groups to present replaceable methodology, policy, guidance, and output templates without changing their document IDs. The project type version advances to `1.0.5` because `1.0.4` was already published in `v0.6.18`.

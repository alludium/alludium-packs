...
Version `0.6.2` separates fund-level Origination Pipeline control from first-class Sourcing Line experiments and Origination Candidate records. It adds a dedicated, integration-aware Origination Manager agent, chat-first line setup, experiment starter configurations, independent line schedules and receipts, candidate-specific screening and outreach state, and namespaced relationship contracts for linking pipelines, lines, candidates, and promoted Deal Pipelines. Existing Origination Pipeline instances remain readable but require the migration described in `docs/origination-project-model-migration.md` before legacy line and candidate state is treated as active automation.
